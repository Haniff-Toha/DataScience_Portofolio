...
async function predict(image) {
    ...
    return new Promise((resolve) => {
        img.onload = async () => {
            ...
            const tensorImg = tf.browser.fromPixels(img).toFloat().div(tf.scalar(255));
            const resizedImg = tf.image.resizeBilinear(tensorImg, [224, 224]).expandDims(0);
            console.log(resizedImg.shape);  // Should print: [1, 224, 224, 3]
            const prediction = model.predict(resizedImg);
            const scores = prediction.dataSync();
            const classIndex = scores.indexOf(Math.max(...scores));
            const classLabels = ['ModerateDemented', 'NonDemented', 'VeryMildDemented', 'MildDemented'];
...
async function init() {
    ...
    model = await tf.loadGraphModel('http://127.0.0.1:5500/Alzheimer_Disease_Brain_MRI_Image_Classification/tfjs_model2/model.json');
    isModelLoaded = true;
